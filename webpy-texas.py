...
urls = (
    ...
    '/googleplay/purchase', 'googleplay_purchase',
    '/verandcfg', 'verandcfg',
    ...
    '/paramida/notify', 'channel_paramida_notify',
    '/mol/notify', 'channel_mol_notify',
...
class iospay_purchase:
    def POST(self):
        try:
            ...
            if status == 21007:#是sandbox数据，连接沙盒测试服务
                # sandbox
                connect = httplib.HTTPSConnection("sandbox.itunes.apple.com")
                headers = {"Content-type": "application/json"}
                connect.request("POST", "/verifyReceipt", jsonStr)
            ...
            myvar = dict(tradeno=strTradeNo, userid=strTradeUser, sku=strSku, result=data, viplevel=nTradeVip)
...
E_PURCHASE_VERIFY       = 10
ES_PURCHASE_VERIFY      = "signature verify"
E_PURCHASE_INVALIDTRADENO = 11
ES_PURCHASE_INVALIDTRADENO = "invalid tradeno"


class googleplay_purchase:
    def GET(self):
            return self.ProcessNotify()

    def POST(self):
            return self.ProcessNotify()

    def ProcessNotify(self):
        try:
            i = web.input(user="", role="", data="", sign="", inventory="0")
            strUserID = i.user
            strRoleName = i.role
            strData = i.data
            strSign = i.sign
            g_logD.info(self.__class__.__name__ + ",user:" + strUserID + ",role:" + strRoleName + ",data:"+strData + ",sign:" + strSign)
...
#            strSign = ''
            if TexasBilling.ggplay_verify(strData, strSign) == False:
                g_log.error(self.__class__.__name__ + ",verify signature fail. user" + strUserID + ",data:" + strData + ",sign:" + strSign)
                return ErrResult(E_PURCHASE_VERIFY, ES_PURCHASE_VERIFY)

            #解析data数据
            dictData = json.loads(strData)
            # strSku = dictData["productId"]
            strTradeNo = dictData["developerPayload"]

            #定单改为存储在数据库中，并从表中查出定单数据
            myvar = dict(payloadid=strTradeNo)
            results = g_db.select('tb_w_payload', myvar, where="PayloadID=$payloadid and Status=0", what="Sku, User, Role, UserData, Channel")
            if len(results) == 0:
                return self.ErrResult(E_PURCHASE_INVALIDTRADENO, ES_PURCHASE_INVALIDTRADENO)
            r = results[0]
            strTradeSku = r.Sku
            strTradeUser = r.User
            strTradeChannel = r.Channel
            nTradeVip = 0
            strSku = strTradeSku

            #调用存储过程进行各种判断，如签名是否已使用过，是否已完成的交易等
            myvar = dict(tradeno=strTradeNo, userid=strTradeUser, sku=strSku, result=strData, viplevel=nTradeVip)
            ret = g_db.query("call pd_w_purchase(@result, @msg, $tradeno, $userid, $sku, $result, $viplevel, @chipspurchased, @chipsnow, @skutype)", vars=myvar)
            results = g_db.query("select @result, @msg, @chipspurchased, @chipsnow, @skutype")
            if len(results) == 0:
                 g_log.error(self.__class__.__name__ + ",procedure purchase no result. user:"+strTradeUser+",sku:"+strSku+",tradeno:"+strTradeNo+",original"+strData)
                 return ErrResult(E_DATANOTFOUND, ES_DATANOTFOUND)
            dictResult = dict(results[0])
            print(dictResult)
            nRet = dictResult['@result']
            ...
            strSkuType = dictResult["@skutype"]
            if nRet != 0 or llChipsPurchased is None or llChipsNow is None:
                 g_log.error(self.__class__.__name__ + ",procedure purchase return:"+str(nRet)+",reason:" + strMsg + ",user:"+strTradeUser+",sku:"+strSku+",tradeno:"+strTradeNo+",original"+strData)
                 if nRet == 2:#同样的定单号，则这笔交易已处理过，直接返回成功，让客户端IAP完成此笔交易（这种情况可能是在充值web没有返回前断开，导致交易没有调用finish完成）
                    strInfo = json.dumps({"code":0,"id":strUserID,"tradeno":strTradeNo,"purchased":llChipsPurchased,"now":llChipsNow}, ensure_ascii=False)
                    g_logD.info(self.__class__.__name__ + "," + str(session.aid) + "," + strInfo)
                    return strInfo
            ...
            #更新定单状态
            DB_UpdatePayloadStatus(strTradeNo, 1)
            DB_UpdatePurchaseResult(strTradeNo, 1)
...
class verandcfg:
    ...
    #获取用户IP所在地
    def GetClientFrom(self,clientIP):
        return TexasConfig.GetClientFrom(clientIP)

        # strUrl = TexasConfig.IP_PARSE_URL + clientIP 
        # print(strUrl)
        # g_logD.info("CMD_GetClientFrom ip:" + clientIP + "...")
        # dictResult= {}
        # connect = None
        # try:
        #     connect = httplib.HTTPConnection(TexasConfig.IP_PARSE_HOST)
        ...
        #     dictResult = json.loads(data,encoding="utf-8")
        # except Exception, e:
        ...
        # g_logD.info("CMD_GetClientFrom ip:" + clientIP + " OK")
        # return dictResult;
    ...
    def ProcessNotify(self):
        try:
            ...
            if data.gzip.isdigit() and int(data.gzip) > 0:
                ...
                bZip = False
            nDeviceType = TexasConfig.DEVICE_TYPE_UNKNOWN
            ...
            #获取用户IP的来源
            strClientIP = web.ctx.ip;

            listCurService= []
            strIpParse= self.GetClientFrom(strClientIP);
            listIpParseAddr= strIpParse.split()

            if len(listIpParseAddr)== 0:
                listCurService= TexasConfig.g_accXmlDic["Default"]
            else:
                print("find country is : " + listIpParseAddr[0])
                if listIpParseAddr[0] == "中国":
                    listCurService= TexasConfig.g_accXmlDic[listIpParseAddr[0]]
                else:
                    listCurService= TexasConfig.g_accXmlDic["Others"]

            # 给字典中添加加速节点信息
            dictConf = json.loads(strJson)
            dictAccNodeInfo= {}
            dictGameMaps= {}
            for dictVal in listCurService:
                if isinstance(dictVal,dict):
                    if(dictVal["Name"]== "List"):
                        dictAccNodeInfo["ListIP"]= dictVal["ToServer"]
                        dictAccNodeInfo["ListPort"]= dictVal["ToPort"]
                    elif(dictVal["Name"]== "Jss"):
                        dictAccNodeInfo["JssIP"]= dictVal["ToServer"]
                        dictAccNodeInfo["JssPort"]= dictVal["ToPort"]
                    elif(dictVal["Name"]== "Chat"):
                        dictAccNodeInfo["ChatIP"]= dictVal["ToServer"]
                        dictAccNodeInfo["ChatPort"]= dictVal["ToPort"]
                elif isinstance(dictVal,list):
                    # 添加加速节点的gamemap
                    game_list= list(dictVal)
                    print(game_list)
                    for game in game_list:
                        dictGameMaps[game["From"]]=  game["To"]
            

            dictAccNodeInfo["ClientIP"]= strClientIP
            dictAccNodeInfo["GameMap"]= dictGameMaps
            dictConf["AccNodeInfo"] = dictAccNodeInfo
            ...
            #根据渠道更新一下配置信息，可能需要提示和更新等
            dCfgFromChannel = TexasConfig.ConfigFromChannel(strChannel, nDeviceType, nVersion)
            if dCfgFromChannel is not None:#需要再进一步处理json
                dictConf = json.loads(strJson)
                dictConf["promptjson"] = dCfgFromChannel["promptjson"]
                if dCfgFromChannel["type"] == TexasConfig.VERSION_UPDATE_FORBIDDEN:
                    dictConf["code"] = E_VERSION_FORBIDDEN
                elif dCfgFromChannel["type"] == TexasConfig.VERSION_UPDATE:
                    dictConf["code"] = E_VERSION_UPDATE
                elif dCfgFromChannel["type"] == TexasConfig.VERSION_PROMPT_FORBIDDEN:
                    dictConf["code"] = E_VERSION_PROMPT_FORBIDDEN
                elif dCfgFromChannel["type"] == TexasConfig.VERSION_PROMPT:
...
class purchase_skus:
    ...
    def ProcessNotify(self):
        try:
            d = web.data()
            print(d)
            i = web.input(gzip="0", userid="", viplevel="0", channel="", market="", paychannel="")
            print(i)
            # if not Loginned():
            #     g_log.info(self.__class__.__name__ + ",not loginned. ip:"+web.ctx.ip)
            ...
            #查找用户是否首充
            bFirstCharge = IsFirstCharge(i.userid)
            nCharged = 1
            if bFirstCharge:
                nCharged = 0

            if len(i.channel) > 1:#如果已指定支付渠道
                strChannel = i.channel
            elif len(session.channel) > 1:#兼容德州在verandcfg中指定渠道
                strChannel = session.channel
            else:
                strChannel = "default"
            strMarket = i.market
            strPayChannel = i.paychannel
            print("channel:" + strChannel + ",market:" + strMarket + ",paychannel:" + strPayChannel)

            #根据渠道号取一些本地支付信息，可用与操作 0：没有支付；1：官方支付；2：本地支付；3：官方和本地支付
            nPayChannel = TexasConfig.GetPayChannel(strChannel, strMarket)

            listSkus = []
            if len(strPayChannel) > 0:#只要指定了支付渠道方式取商品，则不用普通渠道号，为了支持发布渠道和支付渠道的组合，使用发布渠道和支付渠道生成唯一支付渠道ID
                strChannelID = strChannel + "-" + strPayChannel
            if not HaveSkusOfChannel(strChannelID):#此渠道没有单独配置商品数据，使用缺省商品数据
                strChannelID = "default"

            #从数据库中读取商品信息，生成列表
            myvar = dict(viplevel=nVipLevel, channel=strChannelID)
            if nVipLevel > 0:#需要查VIP表
                results = g_db.query("select a.Sku,a.DispOrder,a.Icon,a.Image,a.PriceInfo,a.Price,a.Title,a.Name,a.Type,a.Addition,a.Descript,a.AmountInfo,a.Amount,\
                            a.FirstAddition,a.FirstAmount, a.Addition2, a.Visible, a.Hot, a.AddNum, a.StartTime, a.EndTime, b.VipName,b.VipAddition,b.VipAmount,\
                            b.VipTime,b.VipAddition2 from tb_w_skus a, tb_w_skus_for_vip b where a.sku=b.sku and a.Enable=1 and b.VipLevel=$viplevel and \
                            (now() between a.StartTime and a.EndTime or a.EndTime is null or a.StartTime is null) and \
                            a.sku in (select sku from tb_w_skus_of_channel where channel=$channel) order by a.Price;", vars=myvar)
            else:
                results = g_db.query("select a.Sku,a.DispOrder,a.Icon,a.Image,a.PriceInfo,a.Price,a.Title,a.Name,a.Type,a.Addition,a.Descript,a.AmountInfo,a.Amount,\
                            a.FirstAddition,a.FirstAmount, a.Addition2, a.Visible, a.Hot, a.AddNum, a.StartTime, a.EndTime from tb_w_skus a where a.Enable=1 and\
                            (now() between a.StartTime and a.EndTime or a.EndTime is null or a.StartTime is null) and a.sku in (select sku from tb_w_skus_of_channel where \
                            channel=$channel) order by a.Price;", vars=myvar)
            ...
            for r in results:
                ...
                if not DB_IsExceedLimitToday(r.Sku):
                    ...
                    #     endTime= r.EndTime.strftime("%Y-%m-%d %H:%M:%S") 
                    listSkus.append({"sku":r.Sku,"order":r.DispOrder,"icon":r.Icon,"image":r.Image,"price":r.PriceInfo,"title":r.Title,"name":strName,"type":r.Type,"desc":r.Descript,"addition":strAddition,\
                        "amountinfo":r.AmountInfo,"amount":nAmount,"viptime":nVipTime,"addition2":strAddition2,"visible":r.Visible,"hot":r.Hot,"AddNum":r.AddNum})
            
            dictInfo = {"code":0, "charged":nCharged, "paychannel":nPayChannel, "skus":listSkus}
            strInfo = json.dumps(dictInfo, ensure_ascii=False)
...
class purchase_payload:
    ...
    def ProcessNotify(self):
        try:
            ...
            i = web.input(shop="",userid="",sku="",machineid="",mobile="",userdata="",role="", channel="", paychannel="")
            g_logD.info(self.__class__.__name__ + "," + str(session.aid)+",shop:"+i.shop+",userid:"+i.userid+",sku:"+i.sku+",mid:"+i.machineid+",mobile:"+i.mobile+",role:"+i.role)
            strUser = i.userid
            strRole = i.role
            strSku = i.sku
            strUserData = i.userdata
            strChannelID = i.channel + "-" + i.paychannel
            if i.shop == SHOP_OURGAME:
                ...
                #定单改为存到数据库中，定时删除过期定单
                strInfo = time.asctime()+web.ctx.ip+strUser
                m = md5.new()
                m.update(strInfo)
                strPayload = m.hexdigest()[8:-8].upper()#彼岸只支持16位定单号
                print("Payload:"+strPayload)

                g_db.insert('tb_w_payload', PayloadID=strPayload, Sku=strSku, User=strUser, Role=strRole, UserData=strUserData, Channel=strChannelID, recdate=web.SQLLiteral("NOW()"))

                strInfo = json.dumps({"code":0,"shop":i.shop,"payload":strPayload,"sku":i.sku}, ensure_ascii=False)
                g_logD.info(self.__class__.__name__ + "," + str(session.aid) + "," + strInfo)
                return strInfo
            else:
                return ErrResult(E_PAYLOAD_INVALIDSHOP, ES_PAYLOAD_INVALIDSHOP)
        except Exception, e:
            g_logE.error(self.__class__.__name__ + "," + str(session.aid) + ",exception:" + str(sys.exc_info()[0]) + "," + str(sys.exc_info()[1]))
            fp = StringIO.StringIO()
            traceback.print_exc(file = fp)
...
class channel_ourgame_notify:
    ...
    def ProcessNotify(self, dictRes, strResult):
        try:
            ...
            # strSku = strTradeSku
            myvar = dict(payloadid=strTradeNo)
            # results = g_db.select('tb_w_payload', myvar, where="PayloadID=$payloadid and Status=0 and recdate>NOW()-1800", what="Sku, User, Role, UserData, Channel")
            results = g_db.select('tb_w_payload', myvar, where="PayloadID=$payloadid and Status=0", what="Sku, User, Role, UserData, Channel")
            if len(results) == 0:
                return self.ResultFail("invalid tradeno")
            r = results[0]
            strTradeSku = r.Sku
            strTradeUser = r.User
            strTradeChannel = r.Channel
...
def IsFirstCharge(strUserID):
    if not TexasConfig.DB_ENABLE:
        return False
    if not TexasConfig.SHOP_FIRSTCHARGE_ENABLE:
        return False
...
#更新定单结果
def DB_UpdatePayloadStatus(strTradeNo, nStatus):
    myvar = dict(tradeno=strTradeNo)
    ret = g_db.update('tb_w_payload', vars=myvar, where="PayloadID=$tradeno", status=nStatus)
    if ret == 1:
        return True
    else:
        return False
...
class check:
    def GET(self):
        ...
        try:
            ...
            strInfo = strInfo + "have skus of channel [%s]...\n" % (strChannel)
            bRet = HaveSkusOfChannel(strChannel)
            if bRet:
                strInfo = strInfo + "SUCCESS\n"
            else:
                strInfo = strInfo + "FAIL\n"
...
class monitor:
    def GET(self):
        ...
        try:
            ...
            #检测dbservice
            bRet = CMD_AddPowerGameCoin("supershowplayer", 1)
            dictInfo["dbservice"]= "Fail"
            if bRet:
                dictInfo["dbservice"]= "OK"
            else:
                dictInfo["dbservice"]= "Fail"
...
class channel_paramida_notify:
    ...
    def ProcessNotify(self, d, strResult):
        try:
            if not d.has_key("result") or not d.has_key("orderid") or not d.has_key("amount"):
                return self.ResultFail("no enough data")

            # #解析用户ID，SKU等
            nResult = int(d["result"])
            strTradeNo = d["orderid"]
            nAmount = int(d["amount"])
            strChannel = d["channel_id"]
            strSign = d["sign"]
            g_logD.info(self.__class__.__name__ + ",result:" + str(nResult) + ",orderid:" + strTradeNo + ",amount:" + str(nAmount) + ",channel:" + strChannel + ",sign:" + strSign)
            print("result:" + str(nResult) + ",orderid:" + strTradeNo + ",amount:" + str(nAmount) + ",channel:" + strChannel + ",sign:" + strSign)
            if nResult != 0:
                return self.ResultFail("result fail")
                # return self.ResultSuccess()

            #进行签名比较，防止伪消息
            strSignData = d["orderid"] + d["amount"] + d["result"] + "paramidatech.com"#固定彼岸加密字符串
            m = md5.new()
            m.update(strSignData)
            strSignDataMd5 = m.hexdigest().upper()
            g_logD.info(self.__class__.__name__ + ",sign data md5:" + strSignDataMd5)
            if strSign.upper() != strSignDataMd5:
                return self.ResultFail("invalid sign")

            # listTrade = strTradeNo.split("-")
            # if len(listTrade) < 5:
            #     return self.ResultFail("orderid error")
            # print(listTrade)
            # strTradeChannel = listTrade[0]
            # strTradeUser = listTrade[1]
            # strTradeSku = listTrade[3]
            ...
            #彼岸存在二次选择价格的情况，商品sku需要变化，按照价格再查一次sku表
            strNotifySku = GetSkuFromPrice(nAmount)
            if strNotifySku is None:
                return self.ResultFail("can't find sku from price")
            if strNotifySku != strTradeSku:#二次选择已改变商品，在此替换定单中的sku
                g_logD.info(self.__class__.__name__ + ",notify sku is not equal trade sku! Use notify sku! " + " notify sku:" + strNotifySku + ", trade sku:" + strTradeSku)
                strSku = strNotifySku
            ...
            if len(results) == 0:
                 g_log.error(self.__class__.__name__ + ",procedure purchase no result. user:"+strTradeUser+",sku:"+strSku+",tradeno:"+strTradeNo+",original"+strResult)
                 return self.ResultFail("call procedure fail")
            dictResult = dict(results[0])
            nRet = dictResult['@result']
            strMsg = dictResult['@msg']
            llChipsPurchased = dictResult['@chipspurchased']
            ...
            if nRet != 0 or llChipsPurchased is None or llChipsNow is None:
                 g_log.error(self.__class__.__name__ + ",procedure purchase return:"+str(nRet)+",reason:" + strMsg + ",user:"+strTradeUser+",sku:"+strSku+",tradeno:"+strTradeNo+",original"+strResult)
                 return self.ResultFail("procedure fail:" + strMsg)

            #到此已经增加充值记录，需要根据不同商品去各个服务器给用户加数据
            if strSkuType == SKU_TYPE_COIN:#加万能豆，用的是用户名
                if CMD_AddPowerGameCoin(strTradeUser, llChipsPurchased) == False:
                     g_log.error(self.__class__.__name__ + ",add powergamecoin fail! User:" + strTradeUser + ",coin:" + str(llChipsPurchased))
                     return self.ResultSuccess()#只要记录已加入到这边库中，就返回成功
            elif strSkuType == SKU_TYPE_VIP:#加会员资格，用的是角色名
                nVipType = GetVipType(strSku)
                if 0 == nVipType:
                     g_log.error(self.__class__.__name__ + ",can't find vip type! Role:" + strRoleName + ",sku:" + strSku)
                     return self.ResultSuccess()#只要记录已加入到这边库中，就返回成功
                if CMD_AddVip(strRoleName, nVipType) == False:
                     g_log.error(self.__class__.__name__ + ",add vip fail! Role:" + strRoleName + ",viptype:" + str(nVipType))
                     return self.ResultSuccess()#只要记录已加入到这边库中，就返回成功
            ...
            # #加积分，现在定单里没有角色数据，不能加积分了，需要时得扩展定单号数据
            # if strSkuType == SKU_TYPE_COIN:#只有礼包才需要这边加积分
            #     nScore = DB_ScoreOfSku(strSku)
            #     if CMD_AddScore(strRoleName, 1, nScore) == False:#购买加积分类型是1
            #         g_log.error(self.__class__.__name__ + ",add score fail! Role:" + strRoleName + ",score:" + str(nScore))
            ...
            traceback.print_exc(file = fp)
            g_logE.error(self.__class__.__name__ + "," + fp.getvalue())
            return self.ResultFail("exception")

    def ResultSuccess(self):
        g_logD.info(self.__class__.__name__ + ",success")
        return "success"

    def ResultFail(self, strReason):
        print(strReason)
        g_logD.info(self.__class__.__name__ + "," + strReason)
        return strReason

# mol支付服务端通知
# Application Name: Ourgame_PokerMonster
# Application Code: riv9yrxPhKu67tspcspWWoLXuh1D9mMs
# Secret Key: OFa9dOcmHhR67QPM1PMttLiuRHyV9JgP
MOL_SECRETKEY = "OFa9dOcmHhR67QPM1PMttLiuRHyV9JgP"
class channel_mol_notify:
    def GET(self):
        g_logD.info(self.__class__.__name__ + ", web.ctx.ip="+web.ctx.ip)
        dInput = dict(web.input())
        print(dInput)
    ...
    def ProcessNotify(self, d, strResult):
        try:
            if not d.has_key("paymentStatusCode") or not d.has_key("customerId") or not d.has_key("signature"):
                return self.ResultFail("no enough data")

            # #解析用户ID，SKU等
            strCode = d["paymentStatusCode"].strip()
            # strCustomerId = d["customerId"].strip()
            strTradeNo = d["referenceId"].strip()
            strAmount = d["amount"].strip()
            strChannel = "mol"
            strSign = d["signature"].strip()
            g_logD.info(self.__class__.__name__ + ",result:" + strCode + ",orderid:" + strTradeNo + ",amount:" + strAmount + ",channel:" + strChannel + ",sign:" + strSign)
            print("result:" + strCode + ",orderid:" + strTradeNo + ",amount:" + strAmount + ",channel:" + strChannel + ",sign:" + strSign)
            if strCode != "00":
                return self.ResultFail("result fail")
            ...
            for k in listParams:
                if d[k] == "":#不参与排序
                    continue
                if k == "signature":#不参与排序
                    continue
                strSignData = strSignData + d[k]
            strSignData = strSignData + MOL_SECRETKEY
            g_logD.info(self.__class__.__name__ + ",sign data:" + strSignData)
            ...
            #为了安全，应该对传过来的价格与商品价格做一次比较，因为mol是指定价格支付的 todo
    ...
    def ResultFail(self, strReason):
        print(strReason)
        g_logD.info(self.__class__.__name__ + "," + strReason)
        web.ctx.status='406 Not Acceptable'#不需要返回信息，只要返回非200
        # return strReason

#根据商品价格查找对应商品
def GetSkuFromPrice(fPrice):
    if not TexasConfig.DB_ENABLE:
        return None
    myvar = dict(price=fPrice)
    results = g_db.select('tb_w_skus', myvar, where="Price=$price", what="sku")
    if len(results) == 0:
        return None
    else:
