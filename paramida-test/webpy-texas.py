...
class purchase_skus:
    ...
    def ProcessNotify(self):
        try:
            d = web.data()
            print(d)
            i = web.input(gzip="0", userid="", viplevel="0", channel="", market="")
            print(i)
            # if not Loginned():
            #     g_log.info(self.__class__.__name__ + ",not loginned. ip:"+web.ctx.ip)
            ...
            if len(i.channel) > 1:#如果已指定支付渠道
                ...
                strChannel = "default"
            strMarket = i.market
            print("channel:" + strChannel + ",market:" + strMarket)
            ...
            if nVipLevel > 0:#需要查VIP表
                results = g_db.query("select a.Sku,a.DispOrder,a.Icon,a.Image,a.PriceInfo,a.Price,a.Title,a.Name,a.Type,a.Addition,a.Descript,a.AmountInfo,a.Amount,a.FirstAddition,a.FirstAmount, a.Addition2, a.Visible, a.Hot, b.VipName,b.VipAddition,b.VipAmount,b.VipTime,b.VipAddition2 from tb_w_skus a, tb_w_skus_for_vip b where a.sku=b.sku and a.Enable=1 and b.VipLevel=$viplevel and a.sku in (select sku from tb_w_skus_of_channel where channel=$channel) order by a.Price;", vars=myvar)
            else:
                results = g_db.query("select a.Sku,a.DispOrder,a.Icon,a.Image,a.PriceInfo,a.Price,a.Title,a.Name,a.Type,a.Addition,a.Descript,a.AmountInfo,a.Amount,a.FirstAddition,a.FirstAmount, a.Addition2, a.Visible, a.Hot from tb_w_skus a where a.Enable=1 and a.sku in (select sku from tb_w_skus_of_channel where channel=$channel) order by a.Price;", vars=myvar)
            for r in results:
                # print(r)
                strName = r.Name
                nVipTime = 0
                ...
                if not DB_IsExceedLimitToday(r.Sku):
                    listSkus.append({"sku":r.Sku,"order":r.DispOrder,"icon":r.Icon,"image":r.Image,"price":r.PriceInfo,"title":r.Title,"name":strName,"type":r.Type,"desc":r.Descript,"addition":strAddition,"amountinfo":r.AmountInfo,"amount":nAmount,"viptime":nVipTime,"addition2":strAddition2,"visible":r.Visible, "hot":r.Hot})
...
class purchase_payload:
    ...
    def ProcessNotify(self):
        try:
            ...
            i = web.input(shop="",userid="",sku="",machineid="",mobile="",userdata="",role="")
            ...
            if i.shop == SHOP_OURGAME:
                ...
                g_db.insert('tb_w_payload', PayloadID=strPayload, Sku=strSku, User=strUser, Role=strRole, UserData=strUserData, Channel=strChannel, recdate=web.SQLLiteral("NOW()"))
