...
class login:
    ...
    def POST(self):
        try:
            ...
            #if nVersion < VPub.Version2Int("2.3.0") or (nVersion >= 0x133 and nVersion <= 0x134):
            ...
            #    return strInfo
            # 非强制升级版本要继续下面的流程
            if nVersion < TexasPub.Version2Int("2.3.0"):
                nCode = E_LOGIN_VERSION_UPDATE
             	strPrompt = "{\"prompt\":\"2.3.0版更新内容：\n1、优化牌桌界面；\n2、开通多端互通；\n3、增加在线宝箱功能；\n4、优化了某些交互体验。\", \"url\":\"http://pw.lianzhong.com/Mobile/Download.html\"}"
                #return strPrompt
                #strPrompt = TexasConfig.GetUpdateUrl(strChannel,1)
                #return strPrompt
             	#strPrompt = TexasConfig.URL_UPDATE
...
class iospay_purchase:
    def POST(self):
        try:
            ...
            g_logD.info("user:" + strUserID + ",role:" + strRole + ",result:"+strResult)
            ...
            myvar = dict(tradeno=strTradeNo, userid=strUserID, sku=strSku, result=strResult)
            ret = g_db.query("call pd_w_purchase(@result, @msg, $tradeno, $userid, $sku, $result, @chipspurchased, @chipsnow)", vars=myvar)
            results = g_db.query("select @result, @msg, @chipspurchased, @chipsnow")
            if len(results) == 0:
                 g_log.error(self.__class__.__name__ + "," + str(session.aid) + ",procedure purchase no result. uid:"+strUserID+",sku:"+strSku+",tradeno:"+strTradeNo+",original"+strResult)
            ...
            if nRet != 0 or llChipsPurchased is None or llChipsNow is None:
                 g_log.error(self.__class__.__name__ + "," + str(session.aid) + ",procedure purchase return:"+str(nRet)+",uid:"+strUserID+",sku:"+strSku+",tradeno:"+strTradeNo+",original"+strResult)
